...
public class PublicController {
    ...
    public ResponseEntity<?> getAllIndustry() {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Failed to fetch industries: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<String> reelStatusSet(@RequestBody BarcodeDTO barcodeDTO) {
        try {
            ...
            Order order = orderRepository.findById(barcodeDTO.getOrderId()).orElseThrow(() -> new IllegalArgumentException("Order not found with ID: " + barcodeDTO.getOrderId()));
            ...
            if (existingActiveUsage.isPresent()) {
                ...
                if (currentUsage.getOrder().getId().equals(order.getId())) {
                    return ResponseEntity.status(HttpStatus.CONFLICT).body("Reel " + barcodeId + " is already IN_USE for this order (" + order.getId() + "). No new session started.");
                } else {
    ...
    public ResponseEntity<String> calculateWeight(@RequestBody CalculationDTO calculationDTO) {
        try {
            ...
            OrderReelUsage orderReelUsage = optionalOrderReelUsage.orElseThrow(() -> new IllegalStateException("No active OrderReelUsage found for reel with barcode ID: " + barcodeId));
            ...
            reel.setPreviousWeight(reel.getPreviousWeight());
            ...
            reelUsageHistory.setBoxDetails(String.format("%d x %d x %d", calculationDTO.getLength(), calculationDTO.getWidth(), calculationDTO.getHeight()));
            reelUsageHistory.setUsedBy(calculationDTO.getScannedBy());
            reelUsageHistoryRepository.save(reelUsageHistory);
            return ResponseEntity.ok(String.format("Used Weight = %.2f kg, Current Reel Weight = %d kg. Order %s status: %s. Reel session closed.", usedWeightKg, currentWeight, associatedOrder.getId(), associatedOrder.getStatus()));
    ...
    public ResponseEntity<List<Order>> getOrdersByActiveStatus() {
        List<OrderStatus> activeStatuses = Arrays.asList(OrderStatus.TODO, OrderStatus.IN_PROGRESS);
